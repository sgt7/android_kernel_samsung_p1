...
static DEFINE_PER_CPU(struct greaselightning_info_s, greaselightning_info);
...
enum {
        GREASELIGHTNING_DEBUG_JUMPS=1,
        GREASELIGHTNING_DEBUG_LOAD=2
};
...
static int cpufreq_governor_greaselightning(struct cpufreq_policy *policy,
                unsigned int event);

#ifndef CONFIG_CPU_FREQ_DEFAULT_GOV_GREASELIGHTNING
static
#endif
struct cpufreq_governor cpufreq_gov_greaselightning = {
        .name = "greaslightning",
        .governor = cpufreq_governor_greaselightning,
...
//                        policy->max > sleep_max_freq ? (sleep_max_freq > policy->min ? sleep_max_freq : policy->min) : policy->max;
                ...
                this_greaselightning->max_speed = policy->max;
        }
}

inline static unsigned int validate_freq(struct greaselightning_info_s *this_greaselightning, int freq) {
        if (freq > this_greaselightning->max_speed)
                return this_greaselightning->max_speed;
        if (freq < this_greaselightning->min_speed)
                return this_greaselightning->min_speed;
        return freq;
}

static void reset_timer(unsigned long cpu, struct greaselightning_info_s *this_greaselightning) {
  this_greaselightning->time_in_idle = get_cpu_idle_time_us(cpu, &this_greaselightning->idle_exit_time);
  mod_timer(&this_greaselightning->timer, jiffies + sample_rate_jiffies);
}

static void cpufreq_greaselightning_timer(unsigned long data)
{
        u64 delta_idle;
        u64 delta_time;
        int cpu_load;
	...
	unsigned long new_rate;
        ...
        if (this_greaselightning->idle_exit_time == 0 || update_time == this_greaselightning->idle_exit_time)
                return;

        delta_idle = cputime64_sub(now_idle, this_greaselightning->time_in_idle);
        delta_time = cputime64_sub(update_time, this_greaselightning->idle_exit_time);
        //printk(KERN_INFO "greaselightningT: t=%llu i=%llu\n",cputime64_sub(update_time,this_greaselightning->idle_exit_time),delta_idle);

        // If timer ran less than 1ms after short-term sample started, retry.
        if (delta_time < 1000) {
                if (!timer_pending(&this_greaselightning->timer))
        ...
        if (debug_mask & GREASELIGHTNING_DEBUG_LOAD)
                printk(KERN_INFO "greaselightningT @ %d: load %d (delta_time %llu)\n",policy->cur,cpu_load,delta_time);

        this_greaselightning->cur_cpu_load = cpu_load;

        // Scale up if load is above max or if there where no idle cycles since coming out of idle,
        // or when we are above our max speed for a very long time (should only happend if entering sleep
        // at high loads)
        if ((cpu_load > max_cpu_load || delta_idle == 0) &&
            !(policy->cur > this_greaselightning->max_speed &&
              cputime64_sub(update_time, this_greaselightning->freq_change_time) > 100*down_rate_us)) {

                if (policy->cur > this_greaselightning->max_speed) {
                        reset_timer(data,this_greaselightning);
                }
...
{
        struct greaselightning_info_s *this_greaselightning = &per_cpu(greaselightning_info, smp_processor_id());
        struct cpufreq_policy *policy = this_greaselightning->cur_policy;

        if (!this_greaselightning->enable) {
                pm_idle_old();
                return;
        }

        if (policy->cur == this_greaselightning->min_speed && timer_pending(&this_greaselightning->timer))
                del_timer(&this_greaselightning->timer);
...
static void cpufreq_greaselightning_freq_change_time_work(struct work_struct *work)
{
        unsigned int cpu;
        int new_freq, old_freq;
        unsigned int force_ramp_up;
        int cpu_load;
        struct greaselightning_info_s *this_greaselightning;
        struct cpufreq_policy *policy;
        unsigned int relation = CPUFREQ_RELATION_L;
        cpumask_t tmp_mask = work_cpumask;
        for_each_cpu(cpu, tmp_mask) {
                this_greaselightning = &per_cpu(greaselightning_info, cpu);
                policy = this_greaselightning->cur_policy;
                cpu_load = this_greaselightning->cur_cpu_load;
                force_ramp_up = this_greaselightning->force_ramp_up && nr_running() > 1;
...
static struct attribute * greaselightning_attributes[] = {
...
static struct attribute_group greaselightning_attr_group = {
        .attrs = greaselightning_attributes,
        .name = "greaselightning",
};

static void greaselightning_suspend(int cpu, int suspend)
{
        struct greaselightning_info_s *this_greaselightning = &per_cpu(greaselightning_info, smp_processor_id());
        struct cpufreq_policy *policy = this_greaselightning->cur_policy;
        unsigned int new_freq;

        if (!this_greaselightning->enable || sleep_max_freq==0) // disable behavior for sleep_max_freq==0
                return;
...
static void greaselightning_early_suspend(struct early_suspend *handler) {
        int i;
        for_each_online_cpu(i)
                greaselightning_suspend(i,1);
}

static void greaselightning_late_resume(struct early_suspend *handler) {
        int i;
        for_each_online_cpu(i)
                greaselightning_suspend(i,0);
}
                ...
                unsigned int event)
{
        ...
        int rc;
        struct greaselightning_info_s *this_greaselightning = &per_cpu(greaselightning_info, cpu);
        ...
        case CPUFREQ_GOV_LIMITS:
                greaselightning_update_min_max(this_greaselightning,new_policy,suspended);
                if (this_greaselightning->cur_policy->cur != this_greaselightning->max_speed) {
                        if (debug_mask & GREASELIGHTNING_DEBUG_JUMPS)
                                printk(KERN_INFO "SmartassI: initializing to %d\n",this_greaselightning->max_speed);
                        __cpufreq_driver_target(new_policy, this_greaselightning->max_speed, CPUFREQ_RELATION_H);
                }
                break;

        case CPUFREQ_GOV_STOP:
                del_timer(&this_greaselightning->timer);
                this_greaselightning->enable = 0;
...
static int __init cpufreq_greaselightning_init(void)
{
        unsigned int i;
        struct greaselightning_info_s *this_greaselightning;
        debug_mask = 0;
	...
	rapid_min_cpu_load = RAPID_MIN_CPU_LOAD;
        ...
        for_each_possible_cpu(i) {
                this_greaselightning = &per_cpu(greaselightning_info, i);
                this_greaselightning->enable = 0;
                this_greaselightning->cur_policy = 0;
                this_greaselightning->force_ramp_up = 0;
                this_greaselightning->max_speed = DEFAULT_SLEEP_WAKEUP_FREQ;
                this_greaselightning->min_speed = DEFAULT_AWAKE_MIN_FREQ;
                this_greaselightning->time_in_idle = 0;
                this_greaselightning->idle_exit_time = 0;
                this_greaselightning->freq_change_time = 0;
                this_greaselightning->freq_change_time_in_idle = 0;
                this_greaselightning->cur_cpu_load = 0;
                // intialize timer:
                init_timer_deferrable(&this_greaselightning->timer);
                this_greaselightning->timer.function = cpufreq_greaselightning_timer;
                this_greaselightning->timer.data = i;
        }
  	...
  	down_wq = create_workqueue("kgreaselightning_down");
        ...
        pr_info("[imoseyon] greaselightning enter\n");

        return cpufreq_register_governor(&cpufreq_gov_greaselightning);
}

#ifdef CONFIG_CPU_FREQ_DEFAULT_GOV_GREASELIGHTNING
pure_initcall(cpufreq_greaselightning_init);
#else
module_init(cpufreq_greaselightning_init);
#endif

static void __exit cpufreq_greaselightning_exit(void)
{
        pr_info("[imoseyon] greaselightning exit\n");
        cpufreq_unregister_governor(&cpufreq_gov_greaselightning);
        destroy_workqueue(up_wq);
        destroy_workqueue(down_wq);
}

module_exit(cpufreq_greaselightning_exit);

MODULE_AUTHOR ("Erasmux/imoseyon");
MODULE_DESCRIPTION ("'cpufreq_greaselightning' - A smart cpufreq governor optimized for the G2!");
MODULE_LICENSE ("GPL");
